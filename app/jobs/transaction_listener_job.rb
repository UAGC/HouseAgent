...
class TransactionListenerJob
  ...
  def perform(*args)
    puts "-----------------"
    puts " 成都房产活动 -- checking"
    HouseCrawler.check!
    puts " 成都房产活动 -- check fine"
    puts "-----------------"
  end
end
...
class HouseCrawler
  ...
  class<<self
    def crawl_statistics
      ...
      res = html.css('.blank tr').children
    ...
    def update_statistics(coming)
      Transaction.where(trx_date: coming[0].trx_date).select { |tx| tx.trx_type.start_with? 'stx' }.each(&:destroy)
      coming.each(&:save)
    ...
    def check!
      ...
      puts "House Market check FINISHED"
    end

    private

    def today
      (Time.now.utc+60*60*8).to_date.to_s
    end
